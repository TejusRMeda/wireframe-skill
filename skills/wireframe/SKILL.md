...
4. Build an explicit **element inventory** — a literal checklist, not prose. This is the source of truth Phase 4 verifies against, so list every distinct element you can see, not just the major sections:
   - Overall dimensions and device type (mobile/tablet/desktop) — **use the source's true dimensions, never a screenshot tool's preview size.** `get_screenshot` caps its rendered `width`/`height` to fit within ~1024px on the long edge and reports the true size separately as `original_width`/`original_height` — use those, or a live node's actual `width`/`height` from `get_metadata`. Building the container at the preview's downscaled size looks fine in isolation but produces a wireframe that doesn't align with the real design at matching coordinates (e.g. when placed side-by-side for a before/after comparison, or handed off to someone who overlays it on the real file) — the two are at different absolute scales even though the aspect ratio matches. If you do need to work from a smaller coordinate system for convenience, keep a single scale constant (e.g. `S = trueWidth / workingWidth`) and run every position/size through it, so the true container is dimensionally exact even if intermediate math used easier numbers.
...
**Figma gotcha — `vectorPaths` only supports `M`, `L`, `Q`, `C`, `Z` (and their lowercase/relative forms) — no `A` (elliptical arc).** Reaching for an SVG arc (e.g. a pie-chart wedge or a true circular segment) throws `Failed to convert path. Invalid command at A...` and fails the whole script atomically (no partial nodes — safe to just fix and retry). Approximate arcs with `Q` quadratic curves instead, or use `figma.createEllipse()`/`figma.createStar()` for true circular/radial shapes and combine with boolean operations if a wedge cutout is needed.
...
4. **For Figma, run an out-of-bounds check** as part of that same read-only script: compare each node's `absoluteBoundingBox` against the frame's own `absoluteBoundingBox`. **Check all four edges — `x` and `y` against the frame's origin, AND `x + width` / `y + height` against the frame's far edge.** A check that only tests the origin point misses a wide/tall element whose top-left corner sits validly inside the frame but whose right or bottom edge runs past it (e.g. a button positioned near the frame's edge but sized too wide to fit — no error, it just renders clipped or overhanging). Nodes failing the origin check are a strong signal of the local-vs-frame coordinate mixup described in Phase 3 (usually a multi-point path authored with frame-scale coordinates); nodes failing only the far-edge check are usually a plain layout miscalculation instead.
